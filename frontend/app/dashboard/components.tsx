...
export function PanelOption<T extends string>({ title, options, value, onChange, children }: { title:string; options:readonly T[]; value:T|null; onChange:(v:T)=>void; children?:React.ReactNode }) {
  return (
...
export function PanelFile({ value, onPickFile, onPickFileClick, fileInputRef }: { value:string; onPickFile:(e:React.ChangeEvent<HTMLInputElement>)=>void; onPickFileClick:()=>void; fileInputRef:React.RefObject<HTMLInputElement> }) {
  return (
    <div className="bg-white/90 backdrop-blur rounded-2xl shadow p-6">
...
export function PanelConfig({
  ...
  passActual:string; passNueva:string; passConfirma:string;
  setPassActual:(v:string)=>void; setPassNueva:(v:string)=>void; setPassConfirma:(v:string)=>void;
  passMsg:{ type: "ok" | "err"; text:string } | null;
  onCambioPassword:()=>void;

  apiKissoroVigente:string; apiKissoroNuevo:string; setApiKissoroNuevo:(v:string)=>void; apiKissoroMsg:{ type:"ok"|"err"; text:string } | null;
  apiEnPluralVigente:string; apiEnPluralNuevo:string; setApiEnPluralNuevo:(v:string)=>void; apiEnPluralMsg:{ type:"ok"|"err"; text:string } | null;
  onCambioApis:()=>void;
}) {
  return (
