...
export default function DashboardPage() {
  ...
  const onCambioApis = async () => {
    ...
    if (!token) return;
    try {
      const res = await fetch("/auth/apis", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ kissoro: apiKissoroNuevo || apiKissoroVigente, enplural: apiEnPluralNuevo || apiEnPluralVigente }) });
      if (!res.ok) throw new Error("Error al actualizar APIs");
      setApiKissoroVigente(apiKissoroNuevo || apiKissoroVigente); setApiEnPluralVigente(apiEnPluralNuevo || apiEnPluralVigente);
      setApiKissoroNuevo(""); setApiEnPluralNuevo("");
      setApiKissoroMsg({ type: "ok", text: "API Kissoro actualizado." }); setApiEnPluralMsg({ type: "ok", text: "API En Plural actualizado." });
  ...
  const onCambioPassword = async () => {
    ...
    try {
      const res = await fetch("/auth/change-password", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ old_password: passActual, new_password: passNueva }) });
  ...
  const fmtFecha = (fechaIso: string) => { if (!fechaIso) return "—"; const d = new Date(fechaIso+"T00:00"); if (Number.isNaN(d.getTime())) return "—"; return `${String(d.getDate()).padStart(2,"0")}-${String(d.getMonth()+1).padStart(2,"0")}-${d.getFullYear()}`; };
    ...
    <main className="min-h-screen bg-no-repeat bg-center bg-cover p-4" style={{ backgroundImage:"url(/fondo.png)", backgroundSize:"100% 100%", backgroundRepeat:"no-repeat"}}>
      <div className="mx-auto max-w-7xl grid grid-cols-1 md:grid-cols-[260px_1fr] gap-6">
        ...
        <section className="space-y-6">
          ...
          {menu==="cuenta" && <PanelCuenta />}
          {menu==="fichero" && <PanelFichero />}
  ...
  function PanelDate({ title, value, onChange }: { title:string; value:string; onChange:(v:string)=>void }) {
    return <div className="bg-white/90 backdrop-blur rounded-2xl shadow p-6">
      <h2 className="text-lg font-semibold mb-4">{title}</h2>
      <input type="date" value={value} onChange={e=>onChange(e.target.value)} className="rounded-lg border border-indigo-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
    </div>;
  }
  ...
  function PanelConfig() {
    return <div className="space-y-8 bg-white/90 backdrop-blur rounded-2xl shadow p-6">
  ...
  function ResumenInferior() {
    return <div className="bg-indigo-100/90 rounded-2xl shadow p-6 border border-indigo-200 mt-8">
