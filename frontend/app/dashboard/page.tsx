    ...
    <main className="min-h-screen bg-no-repeat bg-center bg-cover p-4" style={{ backgroundImage:"url(/fondo.png)", backgroundSize:"100% 100%", backgroundRepeat:"no-repeat" }}>
      <div className="mx-auto max-w-7xl grid grid-cols-1 md:grid-cols-[260px_1fr] gap-6">
        ...
        <section className="space-y-6">
          {menu==="formatoImport" && <PanelOption title="Formato Importación" options={FORMATO_IMPORT_OPTS} value={formatoImport} onChange={v=>setFormatoImport(v as typeof FORMATO_IMPORT_OPTS[number])} />}
          {menu==="formatoExport" && <PanelOption title="Formato Exportación" options={FORMATO_EXPORT_OPTS} value={formatoExport} onChange={v=>setFormatoExport(v as typeof FORMATO_EXPORT_OPTS[number])} />}
          {menu==="empresa" && <PanelOption title="Empresa" options={EMPRESAS} value={empresa} onChange={v=>setEmpresa(v as typeof EMPRESAS[number])} />}
          {menu==="proyecto" && <PanelOption title="Proyecto" options={PROYECTOS} value={proyecto} onChange={v=>setProyecto(v as typeof PROYECTOS[number])} />}
          {menu==="cuenta" && <PanelOption title="Cuenta contable" options={CUENTAS} value={cuenta} onChange={v=>setCuenta(v as typeof CUENTAS[number])}>
          ...
          {menu==="fichero" && <PanelFile value={ficheroNombre} onPickFile={onPickFile} onPickFileClick={onPickFileClick} />}
...
export function PanelFile({ value, onPickFile, onPickFileClick }: PanelFileProps) {
  return (
    <div className="bg-white/90 backdrop-blur rounded-2xl shadow p-6">
